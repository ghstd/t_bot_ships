...
export async function hearsHandler(ctx: hearsCTX, coord_1: number, coord_2: number) {
	...
	if (movesCount === 1) {
		...
		if (shipsQuantity === 0) {
			ctx.telegram.sendMessage(movingPlayer.userId, `
					игра завершена,
					победил ${waitingPlayer.userName}
					количество ходов ${movesCount}
				`, Markup.removeKeyboard())

			ctx.telegram.sendMessage(waitingPlayer.userId, `
					игра завершена,
					победил ${waitingPlayer.userName}
					количество ходов ${movesCount}
				`, Markup.removeKeyboard())
		...
		ctx.telegram.sendMessage(waitingPlayer.userId, 'ход второго игрока', Markup.removeKeyboard())
