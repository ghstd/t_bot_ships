...
bot.command('invite', (ctx) => {
    if (playersDb.length > 0) {
        playersDb.forEach((player) => {
            ctx.telegram.sendMessage(player.player.id, 'предыдущая партия завершена', telegraf_1.Markup.removeKeyboard());
        });
    }
    playersDb = [];
    movesCount = 1;
...
bot.hears(fieldTemplate.map((item, index) => item.map((subItem, n) => `${String.fromCharCode((65 + index))}${n}`)).flat(), (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (movesCount === 1) {
        ...
        if (movingPlayer && waitingPlayer) {
            ...
            yield ctx.reply(`второй игрок походил: ${previousPlayerMove}`);
